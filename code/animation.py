...
def draw_agent(draw_obj: ImageDraw, agent: point, scale: int) -> None:
    center = (agent.x * scale + 0.5 * scale, agent.y * scale + 0.5 * scale)
    draw_obj.circle(radius=scale / 2, xy=center, fill=(255, 00, 00, 255), width=0.0)
...
def draw_visible_area(img, agent: point, visible_radius: int, scale: int):
    ...
    bottom_right = (
        (col + visible_radius) * scale - 1,
        (row + visible_radius) * scale - 1,
    )
    draw_obj.rectangle([top_left, bottom_right], fill=(200, 200, 00, 80), width=0.0)
    return overlay
...
def create_animation(dstar_data: json, output_name: str) -> None:
    steps = []
    for json_step in dstar_data["steps"]:
        ...
        predicted_steps = [
            point(substep["x"], substep["y"]) for substep in json_step["predicted_path"]
        ]
        expanded_now = [
            point(substep["x"], substep["y"]) for substep in json_step["expanded_now"]
        ]
    ...
    all_obstacles = [
        point(json_point["x"], json_point["y"])
        for json_point in dstar_data["all_obstacles"]
    ]
    scale = 10
...
def display_path(dstar_data: json, output_name: str):
    ...
    goal_x, goal_y = dstar_data["goal"]["x"], dstar_data["goal"]["y"]
    path = [
        point(json_point["x"], json_point["y"]) for json_point in dstar_data["path"]
    ]
    all_obstacles = [
        point(json_point["x"], json_point["y"])
        for json_point in dstar_data["all_obstacles"]
...
def run(
    ...
    print(["output/main", str(run_name), str(tasknum), str(radius)])
